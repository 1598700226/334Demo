...
public class PointCloudShow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (savePointCloudAndMesh)
        {
            savePointCloudAndMesh = false;
            CreatePreObject();
        }
    }
        ...
        else
        {
            ...
            if (isPointTriangle) {
                ...
                delaunator.ForEachTriangle(Triangle =>
                {
                    ...
                    // 剔除异常面片
                    if (angleAInRadians < 0.2 || angleBInRadians < 0.2 || angleCInRadians < 0.2 || area > 200)
                    {
                        ;
                    }
                    else
    ...
    IEnumerator CreatTriangleMesh(String meshFileName, Vector3[] triangles, Vector3[] verticePoints, Color[] colors)
    {
        ...
        {
            if (!CheckTriangleIsCorrect(i, verticePoints, 0.001, 0.2)) {
                continue;
            }
    ...
    private bool CheckTriangleIsCorrect(Vector3 triangle, Vector3[] verticePoints, double triangle_max_area = 200, double min_angle = 0.2) {

        Vector3 pt1 = verticePoints[(int)triangle.x];
        Vector3 pt2 = verticePoints[(int)triangle.y];
        Vector3 pt3 = verticePoints[(int)triangle.z];
        float a = Vector3.Distance(pt1, pt2);
        float b = Vector3.Distance(pt2, pt3);
        float c = Vector3.Distance(pt1, pt3);
        // 计算三角形的角度值，剔除过于尖锐的三角形
        ...
        double area = Math.Sqrt(s * (s - a) * (s - b) * (s - c));
        // 剔除异常面片
        if (angleAInRadians < min_angle || angleBInRadians < min_angle || angleCInRadians < min_angle || area > triangle_max_area)
        {
            return false;
        }
        return true;
    ...
    private void CreatePreObject() {
        ...
        // Store Mesh
        foreach (MeshFilter meshFilter in pointCloud.GetComponentsInChildren<MeshFilter>()) {
            UnityEditor.AssetDatabase.CreateAsset(meshFilter.mesh, "Assets/Resources/PointCloudMeshes/" + filename + @"/" + meshFilter.gameObject.name + ".asset");
